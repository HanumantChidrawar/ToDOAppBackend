            ...
            UserModel.findOne({ email: req.body.email })
                .exec((err, retrievedUserDetails) => {
                    if (err) {
                        ...
                        newUser.receivedFriendRequests = [];
                        ...
                        newUser.save((err, newUser) => {
                            if (err) {
                                ...
                                mailer.autoEmail(newUserObj.email, `<p>Hi ${newUser.firstName},<p><br><h2>Welcome to ToDo Application</h2><br>
                                <a href='http://todoapp.hanumantpatil.co/verifyUser/${newUser.userId}'>Click here to verify yourself</a><br>
                                <p>If You have not the one who signed up for ToDo App.<strong>Please ignore this mail.</strong></p>`);
                                resolve(newUserObj);
                            }
                        });//end newUser.save()
                    } else {
    ...
    let isVerified = (userDetails) => {
        ...
        return new Promise((resolve, reject) => {
            ...
            if (userDetails.userVerified == false) {
                ...
                let apiResponse = response.generate(true, "User not Verified", 305, null);
                reject(apiResponse);
            }
            else {
            ...
            if (req.body.password) {
                ...
                passwordLib.comparePassword(req.body.password, retrievedUserDetails.password, (err, isMatch) => {
                    if (err) {
                        ...
                        let apiResponse = response.generate(true, "Password is incorrect", 300, null);
                        reject(apiResponse);
                    }
                });
    ...
    if (check.isEmpty(req.params.email)) {
        ...
        UserModel.findOne({ email: req.params.email }, (err, userDetails) => {
            ...
            if (err) {
                ...
                mailer.autoEmail(req.params.email, `<a href='http://todoapp.hanumantpatil.co/resetPassword/${userDetails.userId}'>click here to reset password</a>`);
...
let getUsers = (req, res) => {
    ...
    if (check.isEmpty(req.params.userId)) {
        logger.info('userId parameter missing', 'userController: getUsers', 9)
        let apiResponse = response.generate(true, 'userId parameter missing.', 403, null)
        res.send(apiResponse)
    }
    else {
        UserModel.find({ userId: { $ne: req.params.userId } }, (err, userDetails) => {
            /* handle the error if the user is not found */
            if (err) {
